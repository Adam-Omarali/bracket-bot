...
class RobotController:
    def __init__(self, velocity_broker="localhost", orientation_broker="localhost", velocity=0.1):
        self.state = {"position": 0, "velocity": 0, "desired_velocity": 0, "angle": 0}
    ...
    def on_message(self, client, userdata, msg):
        ...
        self.state["desired_velocity"] = payload.get("desired_velocity", 0)
    ...
    def get_state(self):
        """
        Returns the current state of the robot as [position, velocity, desired_velocity, angle]
        """
        return [self.state["position"], self.state["velocity"], self.state["desired_velocity"], self.state["angle"]]